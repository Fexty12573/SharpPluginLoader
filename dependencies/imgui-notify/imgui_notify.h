...
namespace ImGui
{
	...
	{
		const auto vp_size = igGetMainViewport()->Size;
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			igSetNextWindowPos(ImVec2(vp_size.x - NOTIFY_PADDING_X, vp_size.y - NOTIFY_PADDING_Y - height), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			igBegin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				igPushTextWrapPos(vp_size.x / 3.f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
				...
				if (!NOTIFY_NULL_OR_EMPTY(title))
				{
					...
					igText(title); // Render title text
					was_title_rendered = true;
				}
				else if (!NOTIFY_NULL_OR_EMPTY(default_title))
				{
					...
					igText(default_title); // Render default title text (ImGuiToastType_Success -> "Success", etc...)
					was_title_rendered = true;
				}
				...
				if (!NOTIFY_NULL_OR_EMPTY(content))
				{
					...
					igText(content); // Render content text
				}
	...
	{
		throw std::exception("Font is manually loaded already.");
		std::unreachable();
