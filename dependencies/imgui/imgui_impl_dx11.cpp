...
{
    return igGetCurrentContext() ? (ImGui_ImplDX11_Data*)igGetIO()->BackendRendererUserData : nullptr;
}
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Bind texture, Draw
                ID3D11ShaderResourceView* texture_srv = (ID3D11ShaderResourceView*)ImDrawCmd_GetTexID(pcmd);
...
static void ImGui_ImplDX11_CreateFontsTexture()
{
    ...
    ImFontAtlas_SetTexID(io.Fonts, (ImTextureID)bd->pFontTextureView);
...
void    ImGui_ImplDX11_InvalidateDeviceObjects()
{
    ...
    if (bd->pFontTextureView) { bd->pFontTextureView->Release(); bd->pFontTextureView = nullptr; ImFontAtlas_SetTexID(igGetIO()->Fonts, 0); } // We copied data->pFontTextureView to io.Fonts->TexID so let's clear that as well.
    if (bd->pIB)                    { bd->pIB->Release(); bd->pIB = nullptr; }
    if (bd->pVB)                    { bd->pVB->Release(); bd->pVB = nullptr; }
