...
{
    return igGetCurrentContext() ? (ImGui_ImplDX12_Data*)igGetIO()->BackendRendererUserData : nullptr;
}
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                texture_handle.ptr = (UINT64)ImDrawCmd_GetTexID(pcmd);
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    ImFontAtlas_SetTexID(igGetIO()->Fonts, (ImTextureID)bd->hFontSrvGpuDescHandle.ptr);
}
...
void    ImGui_ImplDX12_InvalidateDeviceObjects()
{
    ...
    SafeRelease(bd->pFontTextureResource);
    ImFontAtlas_SetTexID(io.Fonts, 0); // We copied bd->pFontTextureView to io.Fonts->TexID so let's clear that as well.
}
...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    const auto dxgi = LoadLibraryA("dxgi.dll");
    IM_ASSERT(dxgi != nullptr);

    const auto create_factory = (decltype(CreateDXGIFactory1)*)GetProcAddress(dxgi, "CreateDXGIFactory1");
    IM_ASSERT(create_factory != nullptr);

    IDXGIFactory4* dxgi_factory = nullptr;
    res = create_factory(IID_PPV_ARGS(&dxgi_factory));
    IM_ASSERT(res == S_OK);
