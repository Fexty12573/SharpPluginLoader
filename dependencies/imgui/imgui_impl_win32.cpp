...
//  [X] Platform: Keyboard support. Since 1.87 we are using the io.AddKeyEvent() function. Pass ImGuiKey values to all key functions e.g. igIsKeyPressed(ImGuiKey_Space). [Legacy VK_* values will also be supported unless IMGUI_DISABLE_OBSOLETE_KEYIO is set]
...
//  2022-01-12: Inputs: Maintain our own copy of MouseButtonsDown mask instead of using igIsAnyMouseDown() which will be obsoleted.
...
{
    return igGetCurrentContext() ? (ImGui_ImplWin32_Data*)igGetIO()->BackendPlatformUserData : nullptr;
}
...
static void ImGui_ImplWin32_ProcessKeyEventsWorkarounds()
{
    ...
    if (igIsKeyDown_Nil(ImGuiKey_LeftShift) && !IsVkDown(VK_LSHIFT))
        ImGui_ImplWin32_AddKeyEvent(ImGuiKey_LeftShift, false, VK_LSHIFT);
    if (igIsKeyDown_Nil(ImGuiKey_RightShift) && !IsVkDown(VK_RSHIFT))
        ImGui_ImplWin32_AddKeyEvent(ImGuiKey_RightShift, false, VK_RSHIFT);

    // Sometimes WM_KEYUP for Win key is not passed down to the app (e.g. for Win+V on some setups, according to GLFW).
    if (igIsKeyDown_Nil(ImGuiKey_LeftSuper) && !IsVkDown(VK_LWIN))
        ImGui_ImplWin32_AddKeyEvent(ImGuiKey_LeftSuper, false, VK_LWIN);
    if (igIsKeyDown_Nil(ImGuiKey_RightSuper) && !IsVkDown(VK_RWIN))
...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ...
    const bool is_app_focused = (focused_window && (focused_window == bd->hWnd || ::IsChild(focused_window, bd->hWnd) || igFindViewportByPlatformHandle((void*)focused_window)));
    if (is_app_focused)
    {
    ...
    if (has_mouse_screen_pos)
        if (HWND hovered_hwnd = ::WindowFromPoint(mouse_screen_pos))
            if (ImGuiViewport* viewport = igFindViewportByPlatformHandle((void*)hovered_hwnd))
...
static void ImGui_ImplWin32_UpdateGamepads()
{
    ...
    #define MAP_BUTTON(KEY_NO, BUTTON_ENUM)     { ImGuiIO_AddKeyEvent(&io, KEY_NO, (gamepad.wButtons & BUTTON_ENUM) != 0); }
    #define MAP_ANALOG(KEY_NO, VALUE, V0, V1)   { float vn = (float)(VALUE - V0) / (float)(V1 - V0); ImGuiIO_AddKeyAnalogEvent(&io, KEY_NO, vn > 0.10f, IM_SATURATE(vn)); }
...
#define ImVectorCast(_TYPE, _VEC)  ((ImVectorExt<_TYPE>*)(&_VEC))
...
static BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor, HDC, LPRECT, LPARAM)
{
    ...
    ImGuiPlatformIO& io = *igGetPlatformIO();
    if (info.dwFlags & MONITORINFOF_PRIMARY)
        ImVectorCast(ImGuiPlatformMonitor, io.Monitors)->push_front(imgui_monitor);
    else
        ImVectorCast(ImGuiPlatformMonitor, io.Monitors)->push_back(imgui_monitor);
    return TRUE;
}
...
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    ImVectorCast(ImGuiPlatformMonitor, igGetPlatformIO()->Monitors)->resize(0);
...
{
    if (viewport_id != 0)
        if (ImGuiViewport* viewport = igFindViewportByID(viewport_id))
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (ImGuiViewport* viewport = igFindViewportByPlatformHandle((void*)hWnd))
    {
        switch (msg)
        {
        case WM_CLOSE:
