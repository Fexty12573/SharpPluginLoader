...
void D3DModule::initialize_for_d3d12(const uintptr_t renderer) {
    m_swap_chain = *(IDXGISwapChain**)(renderer + 0x1470);
    ...
    if (FAILED(m_swap_chain->GetDevice(IID_PPV_ARGS(&m_d3d12_device)))) {
        dlog::error("Failed to get D3D12 device during initialization");
        return;
    }

    const auto swap_chain_vft = *(void***)m_swap_chain;
...
void D3DModule::initialize_for_d3d11(const uintptr_t renderer) {
    m_swap_chain = *(IDXGISwapChain**)(renderer + 0x1488);
    if (FAILED(m_swap_chain->GetDevice(IID_PPV_ARGS(&m_d3d11_device)))) {
...
void D3DModule::d3d11_deinitialize_imgui() {
    dlog::debug("Uninitializing D3D11 ImGui");
...
HRESULT D3DModule::d3d12_present_hook(IDXGISwapChain* swap_chain, UINT sync_interval, UINT flags) {
    ...
    bool hook_present = swap_chain == self->m_swap_chain && !self->m_is_inside_present;
    if (hook_present) {
