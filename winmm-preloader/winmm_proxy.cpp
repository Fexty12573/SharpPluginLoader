...
std::wstring GetModuleFileNameWHelper(HMODULE hModule)
{
    std::wstring filename;
    DWORD copied = 0;
    do {
        filename.resize(filename.size() + MAX_PATH);
        copied = ::GetModuleFileNameW(hModule, &filename[0], (DWORD)filename.size());
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
    if (fdwReason == DLL_PROCESS_ATTACH) {
        ...
        // the correct process.
        std::filesystem::path module{ GetModuleFileNameWHelper(NULL) };
